...
export const AdModal: React.FC<AdModalProps> = ({ isOpen, type, onClose, onReward, onUpgrade }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center animate-fade-in">
      {/* Ad Header */}
      <div className="absolute top-0 right-0 p-4 z-20" style={{ paddingTop: 'max(1rem, env(safe-area-inset-top))' }}>
        {canClose ? (
          <button
            onClick={onClose}
      ...
      <div className="absolute top-0 left-0 p-4 z-20" style={{ paddingTop: 'max(1rem, env(safe-area-inset-top))' }}>
        <div className="bg-yellow-500 text-black text-[10px] font-bold px-2 py-1 rounded shadow-sm">
          ADVERTISEMENT
        </div>
      </div>
      ...
      <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 p-8 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-sm">
          <div className="bg-white/10 backdrop-blur-xl p-8 rounded-3xl border border-white/20 w-full shadow-2xl mb-6">
          ...
          <button
            onClick={onUpgrade}
            className="text-white/60 text-xs hover:text-white underline decoration-white/30 hover:decoration-white transition-all"
          >
